{{-- Verificar si hay un elemento en el array de breadcrumbs --}}

@if (count($breadcrumbs))
    <nav class="mb-2 block">
        <ol class="flex flex-wrap text-slate-700 text-sm">
            @foreach ($breadcrumbs as $item)
             <li class="flex item-center">
                {{-- si no es el primer elemento, agregar separador --}}
                @unless ($loop->first)
                    {{-- el span crea el separador --}}
                    <span class="px-2 text-gray-400">
                        /
                    </span>
                @endunless
                {{-- Revisa si existe una llave/propiedad llamada 'href' --}}
                @isset($item['href'])
                    <a href="{{ $item['href'] }}" class="opacity-40 hover:opacity-100 transition">
                        {{ $item['name'] }}
                    </a>
                @else
                    {{ $item['name'] }}
                @endisset
             </li>
            @endforeach
        </ol>
        {{-- El último elemento quiero que esté resaltado --}}
        @if (count($breadcrumbs) > 1)
            <h6 class="font-bold mt-2">
                {{end ($breadcrumbs)['name']}}
            </h6>
        @endif
    </nav>
@endif